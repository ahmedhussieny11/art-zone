...
import { getLocalizedArticle } from "@/lib/localized-entities";
...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  const homeCrumb = t(dict, "blogArticle.breadcrumbHome");
  const blogCrumb = t(dict, "blogArticle.breadcrumbBlog");
  ...
  const breadcrumbLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: homeCrumb, item: siteUrl },
      { "@type": "ListItem", position: 2, name: blogCrumb, item: `${siteUrl}/blog` },
        ...
        <div className="mx-auto max-w-4xl px-6 lg:px-12">
          <FadeIn>
            <nav className="mb-6 flex items-center gap-2 text-xs text-white/40">
              <Link href="/" className="hover:text-gold">{homeCrumb}</Link>
              <span>/</span>
              <Link href="/blog" className="hover:text-gold">{blogCrumb}</Link>
              <span>/</span>
            ...
            <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-white/50">
              ...
              <time dateTime={article.createdAt}>
                {new Date(article.createdAt).toLocaleDateString(dateLocale, { year: "numeric", month: "long", day: "numeric" })}
              </time>
      ...
      <section className="border-t border-warmgray/10 bg-offwhite py-8">
        <div className="mx-auto max-w-3xl px-6 lg:px-12">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-charcoal">{t(dict, "blogArticle.share")}</span>
            <div className="flex gap-3">
              <a
                ...
                aria-label={t(dict, "blogArticle.shareTwitter")}
              >
              ...
              <a
                ...
                rel="noopener noreferrer"
                className="flex h-9 w-9 items-center justify-center rounded-full border border-warmgray/20 text-warmgray transition-colors hover:border-gold hover:text-gold"
                aria-label={t(dict, "blogArticle.shareFacebook")}
              >
              ...
              <a
                ...
                rel="noopener noreferrer"
                className="flex h-9 w-9 items-center justify-center rounded-full border border-warmgray/20 text-warmgray transition-colors hover:border-gold hover:text-gold"
                aria-label={t(dict, "blogArticle.shareWhatsapp")}
              >
      ...
      {related.length > 0 && (
        <section className="bg-white py-16 md:py-24">
          <div className="mx-auto max-w-7xl px-6 lg:px-12">
            <FadeIn>
              <h2 className="mb-10 text-center text-xs font-semibold tracking-[0.2em] text-gold">{t(dict, "blogArticle.related")}</h2>
            </FadeIn>
