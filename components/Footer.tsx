...
interface FooterSettings {
  ...
  socialLinksShow?: Record<string, boolean>;
}
...
export default function Footer() {
  ...
  const visibleSocials = Object.entries(s.socialLinks || {}).filter(
    ([key, url]) => url && s.socialLinksShow?.[key]
  );
        ...
        <div className="grid gap-12 text-center md:grid-cols-3 md:text-right">
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm">
              ...
              <li className="text-offwhite/50">{s.address}</li>
            </ul>
          </div>
        </div>
        ...
        {visibleSocials.length > 0 && (
          <div className="mt-10 flex items-center justify-center gap-4">
            {visibleSocials.map(([key, url]) => (
              <a key={key} href={url} target="_blank" rel="noopener noreferrer" className="flex h-10 w-10 items-center justify-center rounded-full border border-white/10 text-offwhite/50 transition-colors hover:border-gold hover:text-gold">
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d={FOOTER_SOCIAL_ICONS[key]} /></svg>
              </a>
            ))}
          </div>
        )}

        <div className="mt-8 border-t border-white/10 pt-8 text-center text-xs text-offwhite/30">
          &copy; {new Date().getFullYear()} {s.copyrightText}
        </div>
      </div>
    </footer>
  );
